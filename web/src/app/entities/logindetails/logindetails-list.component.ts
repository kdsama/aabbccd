...
import { LogindetailsDetailComponent } from './logindetails-detail.component';
import { LogindetailsService } from './logindetails.service';

@Component({
    moduleId: module.id,
	templateUrl: 'logindetails-list.component.html',
	selector: 'logindetails-list'
})
export class LogindetailsListComponent {

    @Input() header = "Logindetailss...";
    ...
    logindetailsToDelete : Logindetails;

    // basic search criterias (visible if not in 'sub' mode)
    example : Logindetails = new Logindetails();

    // list is paginated
    currentPage : PageResponse<Logindetails> = new PageResponse<Logindetails>(0,0,[]);
    ...
    loadPage(event : LazyLoadEvent) {
        this.logindetailsService.getPage(this.example, event).
            subscribe(
    ...
    onRowSelect(event : any) {
        ...
        this.router.navigate(['/logindetails', id]);
    }
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/logindetails', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let logindetailsToDelete : Logindetails = <Logindetails> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(logindetailsToDelete);
            }
        });
    }

    private delete(logindetailsToDelete : Logindetails) {
        let id =  logindetailsToDelete.id;

        this.logindetailsService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(logindetailsToDelete);
