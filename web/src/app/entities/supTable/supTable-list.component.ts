...
import { SupTableDetailComponent } from './supTable-detail.component';
import { SupTableService } from './supTable.service';

@Component({
    moduleId: module.id,
	templateUrl: 'supTable-list.component.html',
	selector: 'supTable-list'
})
export class SupTableListComponent {

    @Input() header = "SupTables...";
    ...
    supTableToDelete : SupTable;

    // basic search criterias (visible if not in 'sub' mode)
    example : SupTable = new SupTable();

    // list is paginated
    currentPage : PageResponse<SupTable> = new PageResponse<SupTable>(0,0,[]);


    constructor(private router : Router,
        private supTableService : SupTableService,
    ...
    loadPage(event : LazyLoadEvent) {
        this.supTableService.getPage(this.example, event).
            subscribe(
    ...
    onRowSelect(event : any) {
        ...
        this.router.navigate(['/supTable', id]);
    }
    ...
    addNew() {
        if (this.sub) {
            ...
            this.router.navigate(['/supTable', 'new']);
        }
    }

    showDeleteDialog(rowData : any) {
        let supTableToDelete : SupTable = <SupTable> rowData;

        let dialogRef = this.confirmDeleteDialog.open(ConfirmDeleteDialogComponent);
        dialogRef.afterClosed().subscribe(result => {
            if (result === 'delete') {
                this.delete(supTableToDelete);
            }
        });
    }

    private delete(supTableToDelete : SupTable) {
        let id =  supTableToDelete.id;

        this.supTableService.delete(id).
            subscribe(
                response => {
                    this.currentPage.remove(supTableToDelete);
